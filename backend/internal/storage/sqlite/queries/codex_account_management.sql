...
SELECT * FROM codex_account_switches WHERE id = ?;

-- name: GetCodexAccountSwitchByIdempotency :one
SELECT * FROM codex_account_switches WHERE idempotency_key = ?;

-- name: GetActiveCodexAccountSwitch :one
SELECT * FROM codex_account_switches
...
SET phase = sqlc.arg(next_phase), failure_code = sqlc.arg(failure_code),
    cancellation_requested_at = sqlc.narg(cancellation_requested_at),
...
INSERT INTO codex_account_switch_sessions (
    switch_id, session_id, native_session_id, interface_mode, source_generation,
    was_running, stop_state, restart_state, reviewer_was_running,
    reviewer_native_session_id, reviewer_stop_state, reviewer_restart_state
) VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?, ?)
ON CONFLICT DO NOTHING;

-- name: ListCodexAccountSwitchSessions :many
SELECT * FROM codex_account_switch_sessions WHERE switch_id = ? ORDER BY session_id;
...
SET stop_state = sqlc.arg(stop_state), restart_state = sqlc.arg(restart_state),
    target_generation = sqlc.arg(target_generation), error_code = sqlc.arg(error_code),
