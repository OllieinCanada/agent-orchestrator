...
func (c *CodexAccountsController) cancelSwitch(w http.ResponseWriter, r *http.Request) {
	if c.Svc == nil {
		apispec.NotImplemented(w, r, "POST", "/api/v1/agents/codex/account-switches/{switchId}/cancel")
		return
	}
	result, err := c.Svc.CancelCodexAccountSwitch(r.Context(), strings.TrimSpace(chi.URLParam(r, "switchId")))
	if err != nil {
		writeCodexAccountSwitchError(w, r, err)
		return
	}
...
func writeCodexAccountSwitchError(w http.ResponseWriter, r *http.Request, err error) {
	...
	case errors.Is(err, ports.ErrCodexAccountSwitchCancellationUnsafe):
		envelope.WriteAPIError(w, r, http.StatusConflict, "conflict", "CODEX_ACCOUNT_SWITCH_CANCELLATION_UNSAFE", "Codex sessions are already stopping", nil)
