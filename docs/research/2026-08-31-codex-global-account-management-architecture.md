...
Status: implementation architecture for the Codex account-management PR.
...
sequenceDiagram
    ...
    SM->>AO: Close new input and wait for safe boundaries
    SM->>AO: Stop exact running generations
    SM->>AO: Prove AO-owned writers stopped
    ...
    SM->>AO: Resume same AO sessions and native thread IDs
...
- Cancellation is safe only before `stopping_sessions` becomes durable.
